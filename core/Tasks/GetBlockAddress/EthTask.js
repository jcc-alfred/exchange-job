...
try{
    ...
    var job = schedule.scheduleJob('*/15 * * * * *',async()=>{
        ...
        let ethList =  coinList.filter(coin => coin.coin_api_type_id == 2);
        if(!ethList){
            isRun = false;
            return;
        }
        ethList.forEach(async(coin)=>{
            if(!coin.wallet_ip){
                return;
            }
            try{
                    ...
                    assetsList.list.forEach (async (item) =>{
                            try{
                                ...
                                if(account && account.address && account.privateKey){
                                    ...
                                    //修改数据库
                                    await AssetsModel.setBlockAddress({userId:item.user_id,coinId:item.coin_id,blockAddress:account.address,privateKey:privateKey});
                                }
                            }catch(error){
                                console.error(error);
                            }
