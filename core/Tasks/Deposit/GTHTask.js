...
try {
    ...
    schedule.scheduleJob('1 * * * * *', async () => {
        ...
        let [gttCoin] = coinList.filter(coin => coin.coin_api_type_id == 8 && coin.coin_id == 20);
        if (!gttCoin) {
            isRun = false;
            return;
        }
        try {
            let lastProcBlockNum = fs.readFileSync(__dirname + '/GTHBlockNumber', {
                encoding: 'utf-8',
                flag: 'r'
            });
            console.log('lastProcBlockNum', lastProcBlockNum);
            let gthService = new GTHService(gttCoin.wallet_ip);
            let currentBlockNum = await gthService.getBlockNumber();
            console.log('******currentBlockNum:' + currentBlockNum);
            if (parseInt(currentBlockNum) >= parseInt(lastProcBlockNum)) {
                let transactions = await gthService.getTransactionFromBlock(lastProcBlockNum);
                let transList = transactions.filter(tx => tx.token == 'GTH');
                let blockAddrList = transList.map(tx => tx.toAddr);
                ...
                if (blockAddrList && blockAddrList.length > 0) {
                    ...
                    transList.forEach((tx) => {
                        if (tx && tx.toAddr) {
                            let [userAssetsItem] = userAssetsList.filter((userAssetsItem) => userAssetsItem.block_address.toLowerCase() == tx.toAddr.toLowerCase());
                    ...
                    if (ethTxList && ethTxList.length > 0) {
                        await Promise.all(ethTxList.map(async (userTxObj) => {
                            ...
                            try {
                                ...
                                let amount = Utils.checkDecimal(userTxObj.txObj.transferValue, gttCoin.decimal_digits);
                                ...
                                if (!depositItem) {
                                    ...
                                    let fromBlockAddr = userTxObj.txObj.fromAddr;
                                    let toBlockAddr = userTxObj.txObj.toAddr;
                                    //新增充值记录
