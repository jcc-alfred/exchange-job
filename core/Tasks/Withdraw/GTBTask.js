...
try {
    ...
    var job = schedule.scheduleJob(rule, async () => {
        ...
        let [gtbCoin] = coinList.filter(coin => coin.coin_api_type_id == 6 && coin.coin_id == 8);
        if (!gtbCoin || !gtbCoin.wallet_ip) {
            isRun = false;
            return;
        }
        try {
            let withdrawCount = await WithdrawModel.getUnProcListCountByCoinId(gtbCoin.coin_id);
            let pageSize = 100;
            ...
            for (let page = 1; page <= pageCount; page++) {
                //未处理的提现列表
                let withdrawList = await WithdrawModel.getUnProcListByCoinId(gtbCoin.coin_id, page, pageSize);
                ...
                //内部提现转账 start
                let userAssetsList = await AssetsModel.getUserAssetsByBlockAddrListCoinId(transBlockAddrList, gtbCoin.coin_id);
                ...
                if (transWithdrawList && transWithdrawList.length > 0) {
                    await Promise.all(transWithdrawList.map(async (item) => {
                        let totalWalletAmountWei = await ethService.getBalance(gtbCoin.main_block_address);
                        ...
                        let privateKey = CryptoUtils.aesDecode(gtbCoin.main_block_address_private_key);
        ...
        try {
            let ethService = new EthService(gtbCoin.wallet_ip, gtbCoin.wallet_port, gtbCoin.wallet_passphrase);
            let unConfirmList = await WithdrawModel.getUnConfirmWithdrawListByCoinIdList(gtbCoin.coin_id);
            ...
            if (unConfirmList && unConfirmList.length > 0) {
                await Promise.all(unConfirmList.map(async (withdrawItem) => {
                    ...
                    if (txObj && txObj.blockNumber) {
                        let confirmCount = gtbCoin.confirm_count > 0 ? gtbCoin.confirm_count : 12;
                        ...
                        if (confirmStatus == 1) {
                            ...
                            if (res.affectedRows) {
                                //增加用户资产日志
                                let logRes = await AssetsLogModel.addUserAssetsLog(withdrawItem.serial_num, withdrawItem.user_id, withdrawItem.coin_id, gtbCoin.coin_unit, withdrawItem.submit_amount, withdrawItem.balance_amount, 2, 2, '提现');
                                //发送通知
                                UserModel.sendAlert(
                                ...
                                TransferFeesLogModel.addTransferFees(gtbCoin.coin_id, withdrawItem.txid, gtbCoin.coin_id, fees, 'ETH提现手续费');
                            }
                        }
                    }
