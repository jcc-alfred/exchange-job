...
let ETH_Token_ABI = require('../../Base/ABI/ETH_Token_ABI');
                ...
                for (let page = 1; page <= pageCount; page++) {
                    ...
                    if (transWithdrawList && transWithdrawList.length > 0) {
                        ...
                        for (let index in transWithdrawList) {
                            ...
                            let txObj = null;
                            let Token_ABI = ETH_Token_ABI[coin.coin_name.toUpperCase()];
                            if (Token_ABI) {
                                txObj = await ethService.sendTokenSignedTransactionNew(coin.main_block_address, item.to_block_address, item.trade_amount, privateKey, coin.contract_address, coin.token_decimals, Token_ABI);
                            } else {
                                txObj = await ethService.sendTokenSignedTransaction(coin.main_block_address, item.to_block_address, item.trade_amount, privateKey, coin.contract_address, coin.token_decimals);
