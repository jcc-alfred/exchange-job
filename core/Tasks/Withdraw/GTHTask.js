...
try {
    ...
    schedule.scheduleJob('*/15 * * * * *', async () => {
            ...
            for (let page = 1; page <= pageCount; page++) {
                ...
                if (transWithdrawList && transWithdrawList.length > 0) {
                    for (let i in transWithdrawList) {
                        ...
                        try {
                            let txObj = await gthService.sendSignedTransaction(gttCoin.main_block_address, item.to_block_address, item.trade_amount, secret);
                            if (txObj && txObj.id) {
                                // 修改数据库
        ...
        try {
            ...
            let currenctBlockNum = await gthService.getBlockNumber();
            if (unConfirmList && unConfirmList.length > 0) {
                await Promise.all(unConfirmList.map(async (withdrawItem) => {
                    let txObj = await gthService.getTransaction(withdrawItem.txid);
                    if (txObj && txObj.id) {
