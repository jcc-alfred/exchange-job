...
class OrderModel{
    ...
    async getUnProcBonusList(){
        try{
            ...
            let res =  await cnt.execQuery(`select a.coin_exchange_id,b.coin_id,b.exchange_coin_id,buy_user_id,sell_user_id,SUM(buy_fees) as buy_fees,SUM(sell_fees) as sell_fees
    ...
    async addExBonus(userId,coinId,tradeAmount,referral_path){
        try {
            ...
            let isEnableDividend = exMiningConfig.find((item)=>{return item.config_key == 'isEnableDividend'}).config_value == '1' ? true : false;
            let DividendCoinId = parseInt(exMiningConfig.find((item)=>{return item.config_key == 'DividendCoinId'}).config_value);
            let DividendRate = parseFloat(exMiningConfig.find((item)=>{return item.config_key == 'DividendRate'}).config_value);
            let isEnableExMining = exMiningConfig.find((item)=>{return item.config_key == 'isEnableExMining'}).config_value == '1' ? true : false;
            let exMiningLevel = parseInt(exMiningConfig.find((item)=>{return item.config_key == 'exMiningLevel'}).config_value);
            let miningCoinId = parseInt(exMiningConfig.find((item)=>{return item.config_key == 'coinId'}).config_value);
            let L0Rate = parseFloat(exMiningConfig.find((item)=>{return item.config_key == 'L0Rate'}).config_value);
            let L1Rate = parseFloat(exMiningConfig.find((item)=>{return item.config_key == 'L1Rate'}).config_value);
            let L2Rate = parseFloat(exMiningConfig.find((item)=>{return item.config_key == 'L2Rate'}).config_value);
            let L3Rate = parseFloat(exMiningConfig.find((item)=>{return item.config_key == 'L3Rate'}).config_value);
    ...
    async updateOrderByOrderIdList(orderIdList){
        try {
            let cnt = await DB.cluster('master');
            let sql = `update m_order set proc_bonus_status = 2 where order_id in (?)`;
            let res = cnt.execQuery(sql,[orderIdList]);
            cnt.close();
            return res;
        } catch (error) {
            console.error(error);
