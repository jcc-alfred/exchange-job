...
class OTCEntrustModel {
    ...
    async getInvalidOpenOrder(limit = 100) {
        let cnt = await DB.cluster('slave');
        try {
            let sql = 'select id,entrust_id,trigger_type,buy_user_id,sell_user_id,coin_id,coin_amount from m_otc_order where status in (0,1 ) and end_time < now() limit ?';
            let invalid_open_order = await cnt.execQuery(sql, limit);
            return invalid_open_order;
        } catch (e) {
            throw e
        } finally {
            cnt.close();
    ...
    async InvalidOpenOrder(order) {
        ...
        try {
            ...
            // 更新对应的广告的剩余可用额度
            let update_entrust = await cnt.execQuery('update m_otc_entrust set remaining_amount = remaining_amount+? ' +
                'where id =? and total_amount >= remaining_amount+?', [order.coin_amount, order.entrust_id, order.coin_amount]);
            //更新order的状态为3，已过期
            let update_order = await cnt.execQuery('update m_otc_order set status =3 where id = ?', order.id);
